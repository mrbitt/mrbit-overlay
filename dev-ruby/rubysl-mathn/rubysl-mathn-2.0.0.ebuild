...
ruby_add_bdepend "test? ( >=dev-ruby/bundler-1.3 >=dev-ruby/mspec-1.5 >=dev-ruby/rake-10.0 )"
...
all_ruby_install() {
	all_fakegem_install
	_gemsitedir=$(/usr/bin/ruby -rrbconfig -e 'puts RbConfig::CONFIG["sitelibdir"]')
    sitelibdir=${_gemsitedir//site_ruby/gems}
    insinto "${sitelibdir}"/gems/${RUBY_FAKEGEM_NAME}-${RUBY_FAKEGEM_VERSION}/
    doins -r spec
}
