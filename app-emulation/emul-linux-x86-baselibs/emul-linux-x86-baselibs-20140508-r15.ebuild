...
RDEPEND="!<app-emulation/emul-linux-x86-medialibs-10.2
	...
	abi_x86_32? (
		...
		>=media-libs/libmng-1.0.10-r2[abi_x86_32(-)]
		>=sys-libs/pam-1.1.7-r1[abi_x86_32(-)]
		...
		>=dev-lang/python-2.7.6-r1:2.7[abi_x86_32(-)]
...
src_prepare() {
	...
	use ldap || use '!abi_x86_32' || rm -v "${S}"/lib32/libnss_ldap-2.17.so \
		...
		"${S}"/usr/lib32/liblber-2.4.so.2 \
		"${S}"/usr/lib32/liblber-2.4.so.2.9.1 \
		"${S}"/usr/lib32/liblber.so \
