...
src_prepare() {
	#sed -i -e "s#freetype#freetype2#" "${S}"/src/{ttf.c,ttf.h} || Error sed
	#sed -i -e "s#freetype#freetype2#" "${SM}"/fbtruetype/ttf.c || Error sed
...
src_configure() {
	...
	econf \
		...
		--with-essential-libdir=/$(get_libdir)  || die "failed to build splashutils"
}
...
src_install() {
	...
	cd "${SM}"
	emake DESTDIR="${D}" LIB=${LIB} install  || die "failed to build "

	cd "${S}"
	emake DESTDIR="${D}" STRIP="true" install  || die "failed to build "
