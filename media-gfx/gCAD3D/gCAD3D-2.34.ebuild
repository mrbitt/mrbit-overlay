...
src_install() {
	...
	dodir /usr/bin
 	...
 	if [[ $(get_libdir) == lib32 ]]; then
 	    cp -pPR ../binLinux32/*.so "${D}"/usr/$(get_libdir)/${PN} || die "copy failed"
 	    cp -pPR ../binLinux32/plugins "${D}"/usr/$(get_libdir)/${PN} || die "copy failed"
 	    cp ../binLinux32/gCAD3D "${D}"/usr/$(get_libdir)/${PN} || die "copy failed"
 	    ln -sf "${D}"/usr/$(get_libdir)/gCAD3D//binLinux32/gCAD3D "${D}"usr/bin || die 
	 else
	    cp -pPR ../binLinux64/*.so "${D}"/usr/$(get_libdir) || die "copy failed"
	    cp -pPR ../binLinux64/plugins "${D}"/usr/$(get_libdir) || die "copy failed"
	    cp -pPR ../binLinux64/gCAD3D "${D}"/usr/$(get_libdir) || die "copy failed"
	    ln -sf "${D}"usr/$(get_libdir)/gCAD3D "${D}"usr/bin/gcad3d || die "link failed"
     fi 
        
	  dodir /usr/share/{pixmaps,gcad3d}
	  cp -pPR "${S1}"gcad3d/icons ${D}/usr/share/gcad3d/
	  install -D  ../icons/${PN}.xpm ${D}/usr/share/pixmaps/
	...
	if use doc; then
	  ...
	  sed -i "143s|xdg|#xdg|g" gcad_doxygen.sh || die "sed failed" 
